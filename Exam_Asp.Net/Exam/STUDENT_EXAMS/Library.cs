...
namespace SlnExam.Exam
{
    public class Library
    {
        ...
        public void AddSQLDataSource(string QueryStr, SqlDataSource BSource)
        {
            SqlConnection conn = new SqlConnection(connectionString);
            try
            {
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i <= Controls.GetUpperBound(0); i++)
                    {
                        ...
                        if (Controls[i] is DropDownList) ((DropDownList)Controls[i]).SelectedIndex = ((DropDownList)Controls[i]).Items.IndexOf(((DropDownList)Controls[i]).Items.FindByValue(setir[sutun].ToString()));
                           // ddlsample.Items.IndexOf(ddlsample.Items.FindByValue("x"));
                           // ((DropDownList)Controls[i]).Items.IndexOf(((ListItem)((DropDownList)Controls[i]).Items.FindByValue(setir[sutun].ToString())));
